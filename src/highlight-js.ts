...
export function vscodeTheme2HljsTheme(VSCodeTheme: VSCodeTheme) {
  ...
  const themeStyle = getThemeStyle(hljsSelectorScope, VSCodeTheme)
  return themeStyle
}
...
function generateHljsSelectorScope() {
  ...
  Object.entries(HIGHLIGHTJS_SCOPE_MAPPINGS).forEach(
    ([highlightjsScope, VSCodeScope]) => selectorScope[formatHljsSelector(highlightjsScope)] = VSCodeScope
  )
